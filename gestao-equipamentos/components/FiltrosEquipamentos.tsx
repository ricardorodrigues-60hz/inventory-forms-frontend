      ...
      >
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="flex items-center justify-end gap-2.5 pt-2 border-t border-slate-100">
            ...
            {/* Botão Temporário de Sincronização OCS (Gambiarra para atualizar legados com permissão da API) */}
            {onSincronizarLegados && (
              <button
                type="button"
                onClick={onSincronizarLegados}
                disabled={loadingSincronizacao || loading}
                className="inline-flex items-center justify-center gap-1.5 px-3.5 h-[38px] rounded-xl border border-amber-300 bg-amber-50 text-amber-900 text-xs font-bold hover:bg-amber-100 hover:border-amber-400 transition-all cursor-pointer shadow-xs disabled:opacity-50 disabled:cursor-not-allowed"
                title="Executar script de sincronização de IP e Hostname no OCS para equipamentos legados"
              >
                ...
                <span>{loadingSincronizacao ? 'Sincronizando...' : 'Sincronizar IP/Host (Legados)'}</span>
              </button>
            )}
