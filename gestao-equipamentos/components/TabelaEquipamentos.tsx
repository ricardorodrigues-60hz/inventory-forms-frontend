...
export const TabelaEquipamentos: React.FC<TabelaEquipamentosProps> = ({
  ...
  const filteredData = useMemo(() => {
    ...
    if (searchTerm.trim()) {
      ...
      result = result.filter((item) =>
        ...
        (item.cd_mac && item.cd_mac.toLowerCase().includes(term)) ||
  ...
  const sortedData = useMemo(() => {
    return [...filteredData].sort((a, b) => {
      ...
      if (sortField === 'registrado_em' || sortField === 'dt_entrega') {
  ...
  const handleSaveEditWidget = async () => {
    ...
    try {
      // Força sn_inserido_manual = 'S' pois foi editado/corrigido manualmente por um usuário
      await onEditarEquipamento(editingItem.cd_registro, {
        ...editForm,
        sn_inserido_manual: 'S'
      });
      setEditingItem(null);
        ...
        <table className="w-full text-left text-xs">
          <thead className="bg-slate-100/90 border-b border-slate-200 text-slate-600 font-bold select-none text-[11px]">
            <tr>
              ...
              <th
                onClick={() => handleSort('sn_inserido_manual')}
                className="px-2.5 py-2 cursor-pointer hover:bg-slate-200/70 transition-colors text-center"
                title="Origem do registro: Integração ou Manual / Editado"
              >
                <div className="flex items-center justify-center gap-1">
                  <span>Origem</span>
                  {renderSortIcon('sn_inserido_manual')}
                </div>
              </th>
              ...
              paginatedData.map((item) => (
                ...
                >
                  ...
                  <td className="px-2.5 py-2 text-center">
                    <span
                      className={`text-[9px] font-bold px-1.5 py-0.5 rounded-full inline-block ${item.tipo_maquina === 'SLIM'
                  ...
                  {/* Coluna Origem: Manual ('S') ou Automática / Integração ('N') */}
                  <td className="px-2.5 py-2 text-center whitespace-nowrap">
                    {item.sn_inserido_manual === 'S' ? (
                      <span
                        className="inline-flex items-center gap-1 px-1.5 py-0.5 rounded text-[10px] font-bold bg-amber-50 text-amber-700 border border-amber-200 shadow-2xs"
                        title="Registro inserido ou editado manualmente"
                      >
                        Manual
                      </span>
                    ) : (
                      <span
                        className="inline-flex items-center gap-1 px-1.5 py-0.5 rounded text-[10px] font-medium bg-slate-50 text-slate-500 border border-slate-200"
                        title="Registro originado de integração / contrato"
                      >
                        Auto
                      </span>
                    )}
            ...
            <div className="p-5 space-y-4 text-xs max-h-[80vh] overflow-y-auto">
              ...
              <div className="p-3 bg-slate-50 rounded-xl border border-slate-200 space-y-3">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                  ...
                  <div>
                    <label className="block text-[11px] font-semibold text-slate-600 mb-1">Endereço MAC (cd_mac)</label>
                    <input
                      type="text"
                      placeholder="Ex: 00:1A:2B:3C:4D:5E"
                      value={editForm.cd_mac || ''}
                      onChange={(e) => setEditForm({ ...editForm, cd_mac: e.target.value })}
                      className="w-full px-2.5 py-1.5 border border-slate-200 rounded-lg outline-none focus:border-[#008B95] font-mono text-xs bg-white"
                    />
                  </div>
